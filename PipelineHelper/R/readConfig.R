...
#' @param argv an arg.parser object
#' @rdname readConfig
#'
#' @export
writeParams <- function(params, file, ...) {
    params <- argv[4:length(argv)]
    config <- data.frame(as.character(params), row.names=names(params))
    writeConfig(config, file=file, ...)
}
